...
public class ReleaseHistoryRepositoryTest extends AbstractIntegrationTest {
  ...
  public void testFindReleaseHistoryRetentionMaxId() {
    Page<ReleaseHistory> releaseHistoryPage = releaseHistoryRepository
        .findByAppIdAndClusterNameAndNamespaceNameAndBranchNameOrderByIdDesc(APP_ID, CLUSTER_NAME,
            NAMESPACE_NAME, BRANCH_NAME, PageRequest.of(1, 1));
    assertEquals(5, releaseHistoryPage.getContent().get(0).getId());

    releaseHistoryPage = releaseHistoryRepository
        .findByAppIdAndClusterNameAndNamespaceNameAndBranchNameOrderByIdDesc(APP_ID, CLUSTER_NAME,
            NAMESPACE_NAME, BRANCH_NAME, PageRequest.of(2, 1));
    assertEquals(4, releaseHistoryPage.getContent().get(0).getId());

    releaseHistoryPage = releaseHistoryRepository
        .findByAppIdAndClusterNameAndNamespaceNameAndBranchNameOrderByIdDesc(APP_ID, CLUSTER_NAME,
            NAMESPACE_NAME, BRANCH_NAME, PageRequest.of(5, 1));
  ...
  public void testFindFirst100ByAppIdAndClusterNameAndNamespaceNameAndBranchNameAndIdLessThanEqualOrderByIdAsc() {
    ...
    Page<ReleaseHistory> releaseHistoryPage = releaseHistoryRepository
        .findByAppIdAndClusterNameAndNamespaceNameAndBranchNameOrderByIdDesc(APP_ID, CLUSTER_NAME,
            NAMESPACE_NAME, BRANCH_NAME, PageRequest.of(releaseHistoryRetentionSize, 1));
    long releaseMaxId = releaseHistoryPage.getContent().get(0).getId();
    List<ReleaseHistory> releaseHistories = releaseHistoryRepository
    ...
    releaseHistories = releaseHistoryRepository
        .findFirst100ByAppIdAndClusterNameAndNamespaceNameAndBranchNameAndIdLessThanEqualOrderByIdAsc(
            APP_ID, CLUSTER_NAME, NAMESPACE_NAME, BRANCH_NAME, releaseMaxId);
